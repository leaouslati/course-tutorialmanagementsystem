...
function Courses({ isLoggedIn }) {
  ...
  // On mount, read category from URL query param
  useEffect(() => {
    ...
    <div className="min-h-screen bg-[#F4F8FD]">
      ...
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <p className="text-xl text-gray-600 mb-8 text-left font-semibold">
          {category ? `Showing courses in: ${category}` : "Learn. Grow. Achieve."}
        </p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-2">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <RotateCcw
                ...
                role="button"
                tabIndex={0}
                onKeyPress={e => { if (e.key === 'Enter' || e.key === ' ') resetAll(); }}
              />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {courses.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center col-span-full py-16">
              ...
              <button
                onClick={resetAll}
                className="mt-2 px-4 py-2 bg-[#1976D2] text-white rounded shadow hover:bg-[#1565C0] transition"
              >
                Reset Filters
              </button>
            </div>
